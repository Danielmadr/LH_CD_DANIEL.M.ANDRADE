    ...
    try:
        ...
        if missing_cols:
            df_missing = pd.DataFrame(
                0, index=df_input_clean.index, columns=missing_cols
            )
...
def page_prediction():
    ...
    st.markdown(
        "Utilize os modelos treinados para prever a nota IMDB de um filme com base nas suas características."
    )
    ...
    if not available_models:
        st.error(
            "❌ Nenhum modelo encontrado! Verifique se os modelos estão na pasta 'models/'."
        )
        return
        ...
        with st.form("movie_form"):
            ...
            with col1:
                series_title = st.text_input(
                    "Título do Filme", value="", help="Nome do filme"
                )
                released_year = st.number_input(
                    "Ano de Lançamento", min_value=1900, max_value=2025, value=2020
                )
                certificate = st.selectbox(
                    "Certificação",
                    ["G", "PG", "PG-13", "R", "NC-17", "A", "U", "UA", "Unknown"],
                )
                runtime = st.number_input(
                    "Duração (minutos)", min_value=1, max_value=500, value=120
                )

            with col2:
                genre = st.text_input(
                    "Gênero", value="Drama", help="Ex: Drama, Action,Adventure, Comedy"
                )
                meta_score = st.number_input(
                    "Meta Score", min_value=0.0, max_value=100.0, value=70.0, step=0.1
                )
                no_of_votes = st.number_input(
                    "Número de Votos", min_value=1, value=100000
                )
                gross = st.text_input(
                    "Receita Bruta",
                    value="",
                    help="Ex: 100,000,000 (pode deixar vazio)",
                )
            ...
            if submitted:
                ...
                prediction, success, message = predict_movie_rating(
                    available_models[selected_model]["full_path"], movie_data
                )
            ...
            if st.button("🎯 Prever via JSON"):
                if json_input.strip():
                    try:
                        ...
                        prediction, success, message = predict_movie_rating(
                            available_models[selected_model]["full_path"], json_data
                        )
                        ...
                        if success:
                            ...
                            with col_pred2:
                                ...
                                if prediction >= 8.0:
                                    st.success(
                                        "🌟 Excelente! Este filme pode ser um clássico!"
                                    )
                                elif prediction >= 7.0:
                                    st.info("👍 Muito bom! Vale a pena assistir!")
                                elif prediction >= 6.0:
                                    st.warning(
                                        "👌 Bom filme, mas pode não agradar a todos."
                                    )
                                else:
                                    st.error(
                                        "👎 Nota baixa. Pode ter problemas de qualidade."
                                    )
                        else:
                            st.error(message)
