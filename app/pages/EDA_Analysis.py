...
def analise_fatores_faturamento(df: pd.DataFrame):
    ...
    # 5. ANÁLISE DE FILMES DE ALTO FATURAMENTO
    st.subheader("💰 Características dos Filmes de Alto Faturamento")
    
    # Top 10% em faturamento
    threshold_high = df_revenue['Gross'].quantile(0.9)
    high_revenue_films = df_revenue[df_revenue['Gross'] >= threshold_high]
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.metric(
            "Faturamento mínimo (Top 10%)", 
            f"${threshold_high:,.0f}"
        )
        st.metric(
            "Quantidade de filmes", 
            f"{len(high_revenue_films)}"
        )
        
        # Rating médio dos filmes de alto faturamento
        avg_rating_high = high_revenue_films['IMDB_Rating'].mean()
        avg_rating_all = df_revenue['IMDB_Rating'].mean()
        st.metric(
            "Rating médio (Alto faturamento)",
            f"{avg_rating_high:.2f}",
            f"{avg_rating_high - avg_rating_all:+.2f} vs média geral"
        )
    
    with col2:
        # Gêneros mais comuns nos filmes de alto faturamento
        generos_high = high_revenue_films['Genero_Principal'].value_counts().head(5)
        st.write("**Top 5 Gêneros (Alto Faturamento):**")
        for genero, count in generos_high.items():
            percentage = (count / len(high_revenue_films)) * 100
            st.write(f"• {genero}: {count} filmes ({percentage:.1f}%)")
    
    # 6. MATRIZ DE DISPERSÃO AVANÇADA
    st.subheader("📊 Análise Multivariada")
    
    # Scatter plot com múltiplas dimensões
    fig_scatter = px.scatter(
        df_revenue.sample(min(500, len(df_revenue))),  # Limitar para performance
        x='IMDB_Rating',
        y='Gross',
        size='No_of_Votes',
        color='Genero_Principal',
        hover_data=['Series_Title', 'Released_Year'],
        title='Faturamento vs Rating (tamanho = número de votos)',
        labels={'Gross': 'Faturamento (US$)', 'IMDB_Rating': 'Rating IMDB'}
    )
    ...
    # Calcular algumas estatísticas para insights
    if correlacoes:
        top_corr = corr_df.iloc[0]
        top_genre = genero_stats_filtered.index[0] if not genero_stats_filtered.empty else "N/A"
        
        insights = [
            f"🔗 **Maior correlação com faturamento:** {top_corr['Variável']} ({top_corr['Correlação']:.3f})",
            f"🎭 **Gênero com maior faturamento médio:** {top_genre}",
            f"⭐ **Rating médio dos filmes de alto faturamento:** {avg_rating_high:.2f} (vs {avg_rating_all:.2f} geral)",
            f"📈 **{len(high_revenue_films)} filmes** estão no top 10% de faturamento (>${threshold_high:,.0f})"
        ]
    ...
    recommendations = [
        "**Foque na qualidade:** Filmes com ratings mais altos tendem a ter melhor performance financeira",
        "**Escolha do gênero importa:** Alguns gêneros historicamente performam melhor no box office",
        "**Marketing e buzz:** Filmes com mais votos (indicativo de buzz) podem ter melhor faturamento",
        "**Timing de lançamento:** Considere tendências temporais na indústria cinematográfica"
    ]
