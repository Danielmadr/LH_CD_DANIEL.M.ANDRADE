    ...
    for i in range(len(corr.columns)):
        for j in range(i + 1, len(corr.columns)):
            ...
            if not pd.isna(corr_val):
                corr_pairs.append(
                    {"Variável 1": var1, "Variável 2": var2, "Força": abs(corr_val)}
                )
    if corr_pairs:
        corr_df = (
...
def plot_top_recommendations(df: pd.DataFrame):
    ...
    popular = high_rated[
        high_rated["No_of_Votes"] >= high_rated["No_of_Votes"].quantile(0.7)
    ]
...
def analise_fatores_faturamento(df: pd.DataFrame):
    ...
    st.markdown(
        f"**Dados disponíveis:** {len(df_revenue):,} filmes com informações de faturamento"
    )
    ...
    for col in colunas_numericas:
        if col in df_revenue.columns:
            ...
            if not pd.isna(corr):
                correlacoes.append(
                    {"Variável": col, "Correlação": corr, "Abs_Correlação": abs(corr)}
                )

    if correlacoes:
        corr_df = pd.DataFrame(correlacoes).sort_values(
            "Abs_Correlação", ascending=False
        )
    ...
    rating_stats = (
        df_revenue.groupby("Rating_Faixa", observed=True)["Gross"]
    ...
    genero_stats = (
        df_revenue.groupby("Genero_Principal")["Gross"]
    ...
    decada_stats = (
        df_revenue.groupby("Decada")["Gross"].agg(["count", "mean", "median"]).dropna()
    )
...
def analise_overview_insights(df: pd.DataFrame):
    ...
    df_overview = df_overview[df_overview["Overview"].str.len() > 10]
    ...
    st.markdown(
        f"**Dados disponíveis:** {len(df_overview):,} filmes com descrições válidas"
    )
    ...
    with col1:
        st.metric(
            "Comprimento médio", f"{df_overview['overview_length'].mean():.0f} chars"
        )

    with col2:
        st.metric(
            "Palavras médias", f"{df_overview['overview_words'].mean():.0f} palavras"
        )

    with col3:
        st.metric(
            "Descrição mais longa", f"{df_overview['overview_length'].max()} chars"
        )

    with col4:
        st.metric(
            "Descrição mais curta", f"{df_overview['overview_length'].min()} chars"
        )
    ...
    for genre in top_genres:
        genre_overviews = df_overview[df_overview["Genero_Principal"] == genre][
            "Overview"
        ]
        genre_words = []
            ...
            with genre_tabs[i]:
                if genre in genre_keywords and genre_keywords[genre]:
                    genre_df = pd.DataFrame(
                        genre_keywords[genre], columns=["Palavra", "Frequência"]
                    )
    ...
    st.markdown(
        "Visualização das palavras mais frequentes em forma de nuvem para cada gênero de filme."
    )
    ...
    for genre in genres_of_interest:
        if (
            genre in df_overview["Genero_Principal"].values
            and genre not in top_5_genres
        ):
            ...
            with wordcloud_tabs[i]:
                ...
                if len(genre_text_clean.strip()) > 0:
                    try:
                        ...
                        with col2:
                            st.metric(
                                "Comprimento médio descrição", f"{avg_length:.0f} chars"
                            )
                        ...
                        st.warning(
                            f"Não foi possível gerar nuvem de palavras para {genre}: {str(e)}"
                        )
                else:
                    st.warning(
                        f"Texto insuficiente para gerar nuvem de palavras para {genre}"
                    )
...
def page_analysis():
    ...
    st.markdown(
        "Explore os dados dos filmes através de análises detalhadas e visualizações interativas."
    )
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        [
            ...
            "📊 Análise das Visões Gerais dos Filmes",
        ]
    )
    ...
    with tab1:
        ...
        st.markdown(
            "Veja como as principais variáveis numéricas dos filmes se relacionam entre si. A matriz de correlação abaixo destaca as relações mais fortes, tanto positivas quanto negativas, que podem influenciar o desempenho e a recepção dos filmes."
        )
    ...
    with tab2:
        ...
        st.markdown(
            "Confira os filmes mais recomendados, considerando tanto a avaliação do público quanto a popularidade. Os gráficos a seguir mostram a distribuição dos filmes populares e os gêneros mais frequentes entre eles."
        )
    ...
    with tab3:
        ...
        st.markdown(
            "Nesta seção, vamos explorar os fatores que influenciam o faturamento dos filmes."
        )
    ...
    with tab4:
        ...
        st.markdown(
            "Nesta seção, vamos explorar as visões gerais dos filmes, incluindo aspectos como direção, elenco e produção."
        )
